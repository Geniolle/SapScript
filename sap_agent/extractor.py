...
_BELNR_RE = re.compile(r"\b(?:BELNR|DOCUMENTO|DOC(?:UMENT)? NUMBER)[:\s-]+(\d{4,10})", re.IGNORECASE)
...
_USER_RE = re.compile(r"\b(?:USER|USU[AÁ]RIO|UTILIZADOR)[:\s-]+([A-Z0-9_\-.]{3,40})", re.IGNORECASE)
...
def extract_signal(ticket: TicketContext) -> SapErrorSignal:
    ...
    return SapErrorSignal(
        ...
        document_number=_first(_BELNR_RE, text),
        fiscal_year=_first(_GJAHR_RE, text),
        job_name=_first(_JOB_RE, text),
        user=_first(_USER_RE, text),
