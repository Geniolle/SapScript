# -*- coding: utf-8 -*-
#
# COPIA WEB de 'H. CUA_ADICIONAR.py' adaptada para o cockpit:
#  - executar(ambiente_cockpit, caminho_ficheiro=..., modo_nao_interativo=..., pedir_confirmacao=...)
#  - sem popup Tkinter quando o caminho vem do worker; sem input() [S/N] em modo cockpit
#  - devolve dict {ok, status, message, resumo}
# A logica SU10/SU01 e' identica ao original.
...
NOME_SHEET = "CUA_ADICIONAR"  # sheet fixa (copia web sem prefixo no nome)
...
def _resumir(df_proc):
    """Sumario STATUS -> {total, sucesso, erro, aviso} para devolver ao cockpit."""
    try:
        col = df_proc["STATUS"].astype(str).str.strip().str.upper()
    except Exception:
        return {"total": 0}
    total = len(col)
    sucesso = int((col.str.startswith("S")).sum())
    erro = int((col.str.startswith("E") | col.str.startswith("A")).sum())
    aviso = int((col.str.startswith("W")).sum())
...
def executar(
    ...
    if df_pend.empty:
        return {"ok": True, "status": "SEM_PENDENTES", "message": "Nenhuma linha pendente para processar.", "resumo": {"total": 0}}
    ...
    msg = (
        ...
        f"{resumo.get('erro', 0)} erro(s) de {resumo.get('total', 0)}."
    )
    if not ok_save:
        msg += " AVISO: falha ao gravar o Excel de resultados."
    return {
        "ok": bool(ok_save) and resumo.get("erro", 0) == 0,
        "status": "SUCESSO" if (ok_save and resumo.get("erro", 0) == 0) else "PARCIAL",
        "message": msg,
        "resumo": resumo,
    }
...
if __name__ == "__main__":
    ...
    print(_json.dumps(executar("CUA"), ensure_ascii=False, default=str))