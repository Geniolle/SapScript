...
def executar(
    ...
    for rr in roles_agrupadas:
        print(f" - {rr['AGR_NAME']}: {rr['TEXT']} (Funções Componentes: {len(rr['ROLES_LIST'])})")
    ...
    with Progress(
        ...
        task_roles = progress.add_task("A processar roles compostas...", total=total_roles)

        for idx, rr in enumerate(roles_agrupadas, start=1):
            nome, desc = (rr["AGR_NAME"] or "").strip(), (rr["TEXT"] or "").strip()
            roles_filhas = rr["ROLES_LIST"]

            progress.update(task_roles, description=f"A processar role composta: {nome}")

            print("\n======================================================================")
            print(f"▶ [{idx}/{len(roles_agrupadas)}] INICIANDO ROLE COMPOSTA: {nome} | Componentes: {len(roles_filhas)}")
            print("======================================================================")

            tempo_inicio_role = time.time()

            try:
                print("\n[Etapa 1] Preparação e Dados")
                pfcg.open()
