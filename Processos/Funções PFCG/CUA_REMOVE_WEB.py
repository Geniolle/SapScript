# -*- coding: utf-8 -*-
#
# COPIA WEB de 'J. CUA_REMOVE.py' adaptada ao cockpit (ver CUA_ADICIONAR_WEB.py):
#  - executar(ambiente_cockpit, caminho_ficheiro=..., utilizador=..., agr_name=..., subsystem=...)
#  - MODO_DEBUG_PASSO_A_PASSO=False; sem popup Tkinter; devolve dict {ok,status,message,resumo}
...
NOME_SHEET = "CUA_REMOVE"  # sheet fixa (copia web sem prefixo)
...
MODO_DEBUG_PASSO_A_PASSO = False  # copia web: sem pausas interativas
...
def executar(
    ...
    """Entrada compativel com o cockpit web (ver CUA_ADICIONAR_WEB.py). Dois modos:
    EXCEL (caminho_ficheiro) ou INDIVIDUAL (utilizador+agr_name+subsystem).
    ...
    print(f"📄 {NOME_SCRIPT} | Modo: {'INDIVIDUAL' if individual else 'EXCEL'} | Ambiente: {ambiente}")

    caminho = None
    if individual:
        u, a, sub = (str(x or "").strip().upper() for x in (utilizador, agr_name, subsystem))
        ...
        if df is None:
            return {"ok": False, "status": "ERRO", "message": f"Nao foi possivel ler a sheet '{NOME_SHEET}' de {caminho}"}

    sistema_desejado = MAPA_SISTEMA.get(ambiente)
    if not sistema_desejado:
        return {"ok": False, "status": "ERRO", "message": f"Ambiente invalido: {ambiente}. Use: {', '.join(MAPA_SISTEMA.keys())}"}

    session = conectar_sap(sistema_desejado)
    if not session:
        return {"ok": False, "status": "ERRO", "message": f"Sessao SAP GUI para '{sistema_desejado}' ({ambiente}) nao encontrada. Abra a ligacao no SAP Logon."}
    ...
    if individual:
        ...
        return {
            ...
            "message": f"{r.get('UTILIZADOR')} -x {r.get('AGR_NAME')} ({ambiente}): {st or 'sem status'}" + (f" | {detalhe}" if detalhe else ""),
    ...
    resumo = _resumir(df_final)
    return {
        "ok": resumo.get("erro", 0) == 0,
        "status": "SUCESSO" if resumo.get("erro", 0) == 0 else "PARCIAL",
        "message": f"Remover Utilizador Funcao ({ambiente}): {resumo.get('sucesso', 0)} OK, {resumo.get('erro', 0)} erro(s) de {resumo.get('total', 0)}.",
        "resumo": resumo,
    }
