...
@app.post("/api/salsa-it-agent/pfcg/transaction/roles")
def api_salsa_it_pfcg_transaction_roles(payload: SalsaItPfcgTransactionRolesRequest) -> JSONResponse:
    ...
    return _json_no_store({"job_id": job["id"], "state": job["state"], "tcode": tcode})


@app.get("/api/salsa-it-agent/pfcg/transaction/roles/{job_id}")
def api_salsa_it_pfcg_transaction_roles_job(job_id: str) -> JSONResponse:
    try:
        job = get_job(job_id)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
    ...
    if not job:
        raise HTTPException(status_code=404, detail=f"Job {job_id} não encontrado.")
    if job.get("task") != "pfcg_transaction_roles":
        raise HTTPException(status_code=400, detail="O job indicado não pertence à análise de transação PFCG.")
